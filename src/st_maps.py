...
parser.add_argument('DATASET', type=str, default='VIPL-HR-V1', help='Dataset to generate ST maps for')
...
config.SOURCE_PATH = f"/scratch/zce*/{config.DATASET}/"                         # NOTE: change "/scratch/zce*/" to where your datasets are located
config.ST_MAPS_PATH = f"{config.SOURCE_PATH}st_maps_{config.CLIP_SIZE}c_{config.STRIDE}s/"
if config.DATASET == 'VIPL-HR-V1':
    config.FRAME_RATE = 25.0
elif config.DATASET == 'UBFC-rPPG':
    config.FRAME_RATE = 30.0
elif config.DATASET == 'PURE':
    config.FRAME_RATE = 30.0
elif config.DATASET == 'MPSC-rPPG':
    config.FRAME_RATE = 30.0
...
def process_video(video_path, gt_path):
    ...
    if selected_frame_indices is None:
        ...
        # NOTE: change the following line for other datasets, depending on their file structure
...
def process_videos():
    ...
    # NOTE: define paths to .mp4 or .avi video samples and ground truth HR data appropriately for each dataset
    if config.DATASET == 'VIPL-HR-V1':
    ...
    with parallel_backend("loky", inner_max_num_threads=4):
        ...
        # if you want to process a single video for testing, uncomment the line below and provide appropriate video and ground truth paths
        # Parallel(n_jobs=1)([delayed(process_video)('/scratch/zce*/VIPL-HR-V1/p57/v1/source1/video.avi', '/scratch/zce*/VIPL-HR-V1/p57/v1/source1/gt_HR.csv')])
    end = time.time()
