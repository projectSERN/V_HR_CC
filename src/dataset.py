...
class DatasetHR(Dataset):
    """ Dataset class for train / val / test set videos (split into clips here).
    ...
    def __getitem__(self, index):
        ...
        # pass frame rate depending on dataset
        if 'VIPL-HR-V1' in st_map_path:
            frame_rate = torch.tensor(config.FRAME_RATE_VIPL, dtype=torch.float)
        elif 'UBFC-rPPG' in st_map_path:
            frame_rate = torch.tensor(config.FRAME_RATE_UBFC, dtype=torch.float)
        elif 'PURE' in st_map_path:
            frame_rate = torch.tensor(config.FRAME_RATE_PURE, dtype=torch.float)
        elif 'MPSC-rPPG' in st_map_path:
            frame_rate = torch.tensor(config.FRAME_RATE_MPSC, dtype=torch.float)

        return {
            "st_map": st_map,           # 3D tensor of shape: (T, n, c)
            "target": target_hr,        # 1D tensor of shape: (1,)
            "file_path": st_map_path,   # config.ST_MAPS_PATH/.../st_maps.npy of this clip
            "frame_rate": frame_rate    # 1D tensor of shape: (1,)
        }